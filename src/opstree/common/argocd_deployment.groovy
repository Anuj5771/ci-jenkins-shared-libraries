...
def eks_deployment(Map step_params) {
    ...
    eks_api_endpoint = (eksEnvName && env[eksEnvName]) ? env[eksEnvName] : 'https://kubernetes.default.svc'
    ...
    dir("${WORKSPACE}/${repo_dir}") {
        withCredentials([string(credentialsId: argocd_credential_id, variable: 'PASSWORD')]) {
            ...
            logger.logger('msg':'ArgoCD operation completed.', 'level':'INFO')
            ...
            if (!dry_run && perform_health_check) {
                ...
                logger.logger('msg':'Dry run completed. Skipping sync and health check.', 'level':'INFO')
            } else {
